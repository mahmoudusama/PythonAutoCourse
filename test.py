...
def fuzzy_math(num1, operator, num2):
    if type(num1) != int or type(num2) != int:
        raise Exception('We need to do fuzzt math on ints')
...
class TestFuzzyMath:
    ...
    def test_non_int_input_for_num2(self):
        pass
    def test_additional_with_negative_result(self):
        pass
    def test_additional_with_small_result(self):
        assert 'small number' in fuzzy_math(2,'+',2)
    def test_additional_with_medium_result(self):
        assert 'medium-sized' in fuzzy_math(9,'+',9)
    def test_additional_with_large_result(self):
